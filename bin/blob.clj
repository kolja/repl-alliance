...
(-> (create-ns 'unrepl.repl$nPaqzHkkPFtk9aFtDagvVGkQNUU)
(intern '-init-done)
(alter-var-root
(fn [v]
...
unrepl.printer$llFCwLB0nvVeL_D8e_DdtG0ZQzM
(:require
...
(edn-str x)))
(ns
unrepl.repl$nPaqzHkkPFtk9aFtDagvVGkQNUU
(:require
[clojure.main :as m]
[unrepl.core :as unrepl]
[unrepl.printer$llFCwLB0nvVeL_D8e_DdtG0ZQzM :as p]
[clojure.edn :as edn]
...
(unrepl.printer$llFCwLB0nvVeL_D8e_DdtG0ZQzM.WithBindings.
(select-keys (some-> session-id session :bindings) [#'*print-length* #'*print-level* #'unrepl/*string-length* #'p/*elide*])
(cond
(instance? unrepl.printer$llFCwLB0nvVeL_D8e_DdtG0ZQzM.ElidedKVs x) x
(string? x) x
(instance? unrepl.printer$llFCwLB0nvVeL_D8e_DdtG0ZQzM.MimeContent x) x
:else (seq x)))
p/unreachable))
...
(unrepl.repl$nPaqzHkkPFtk9aFtDagvVGkQNUU/start (clojure.edn/read {:default tagged-literal} *in*))
{}
